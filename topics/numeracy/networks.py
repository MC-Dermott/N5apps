...
_SCENARIOS = [
    {"subject": "A garage is servicing a car engine.", "unit": "hours",
     "tasks": ["Order replacement parts", "Disconnect the battery", "Drain the old oil",
               ...
               "Road-test the car", "Valet the car before collection"]},
    ...
    {"subject": "A team is renovating an old croft house.", "unit": "days",
     "tasks": ["Clear the site", "Repair the roof", "Rewire the electrics", "Replumb the bathroom",
               ...
               "Fit the kitchen", "Landscape the garden", "Carry out a final inspection"]},
    ...
    {"subject": "A company is fitting out a new cafe.", "unit": "days",
     "tasks": ["Strip out the old fittings", "First-fix the electrics", "First-fix the plumbing",
               ...
               "Paint the walls", "Fit the furniture", "Deep clean before opening", "Train the new staff"]},
    {"subject": "A company is repairing a house after a flood.", "unit": "days",
     "tasks": ["Electrical safety tests", "Remove damaged kitchen", "Dry out house",
               "Order and deliver new kitchen and bathroom", "Remove damaged bathroom",
               "Remove damaged floors", "Replace the floors", "Plaster the walls",
               "Paint and decorate walls", "Reinstate bathroom", "Reinstate kitchen"]},
]
...
def generate_networks_pert():
    """Practise the core forward/backward-pass mechanic: fill in the interactive PERT
    diagram, then answer for one specific task's Earliest Start Time or Latest Finish Time."""
...
def generate_networks_exam_style():
    ...
    sub_type = random.choice(["float", "critical_path", "project_duration"])

    if sub_type == "float":
        target = random.choice(net["non_critical"])
        scaffold_steps = [
            {"prompt": f"What is the Earliest Start Time of task {target}?", "answer": net["est"][target]},
            {"prompt": f"What is the Latest Start Time of task {target}? (LFT − duration)",
             "answer": net["lst"][target]},
        ...
        final_prompt = f"(c) State the float time for task {target}, in {scenario['unit']}."
        correct_answer = net["float"][target]
    elif sub_type == "critical_path":
        chain = net["chain"]
        scaffold_steps = [
        ...
        final_prompt = ("(c) State the critical path, giving your answer as task letters "
        ...
        worked.append(f"Total project time = max({', '.join(str(net['eft'][t]) for t in sinks)}) "
                       f"= {net['project_duration']}")
        final_prompt = ("(c) State the minimum total time required to complete the whole job, "
                         f"in {scenario['unit']}.")
...
def generate_networks_question():
    return random.choice([
        generate_networks_pert, generate_networks_gantt, generate_networks_exam_style,
    ])()
